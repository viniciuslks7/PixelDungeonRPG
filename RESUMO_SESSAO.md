...
Ultima Modificação: 2026-04-13

## Atualizacao 2026-04-13 - Geração de Arte e Extensão da Pipeline API

**Contexto:** O projeto necessitava processar artes brutos em spritesheets da Godot (AnimatedSprite2D) e contornar a ausência de modelos ComfyUI locais.

Foi consolidado nesta rodada:

- script de lote Python (`comfy_batch.py`) criado acoplado ao `workflow_api.json` na porta 8188 para automatização da API de imagem.
- processamento de heróis concluído e salvo em `assets/characters/players/`:
  - `spr_archer_sheet.png` transformado em grid de 15 frames
  - `spr_mage_sheet.png` transformado em grid de 14 frames
  - `spr_warrior_humano_sheet.png` transformado em grid de 14 frames
- sprites isolados de monstros gerados nativamente por bypass (Gemini Imagens) e rigidamente ajustados para 32x32 (Pixel Art transparente) em `assets/characters/enemies/`:
  - `spr_enemy_bat_idle.png`
  - `spr_enemy_skeleton_idle.png`
  - `spr_enemy_slime_idle.png`

Arquivos-chave alterados nesta rodada:
- `assets/characters/players/*` e `assets/characters/enemies/*`
- `assets/pipeline/comfy_batch.py`
- `assets/pipeline/workflow_api.json`
- `assets/pipeline/pipeline_report.json`

Proximo passo recomendado p/ agentes:
- Os recursos (Resources) de monstro (`Slime`, `Esqueleto`, `Morcego`) ja podem importar o novo `idle_sprite` como textura de substituição imediata do placeholder.
- O mesmo vale para as classes de Jogador, que devem ser fatiadas com Hframes listados acima.
